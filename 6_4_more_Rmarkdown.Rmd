...
```{r }
res <- t.test(bwt ~ smoke, data = d)
mean_dif <- res$estimate %>% diff() %>% round(1) %>% as.character()
mean_dif_ci <- res$conf.int %>% round(1) %>% paste(collapse=", ") 
md_ci <- paste0(mean_dif, " [95%CI: ", mean_dif_ci, "]")
p <- res$p.value %>% scales::pvalue()
```

The mean weight of newborns was lower in smoker than in non-smoker mothers (p = `` `r knitr::inline_expr("p")` ``), by `` `r knitr::inline_expr("md_ci")` `` grams.

Becomes:

The mean weight of newborns was lower in smoker than in non-smoker mothers (p = `r p`), by `r md_ci` grams.


## Exercise

Update [this .Rmd file](https://acobos.github.io/CDB_ed2/6_3_rmarkdown_exe_1_solution.Rmd) of the analysis of birth weights:
