...
gf_point(fct_reorder(region, mean_age) ~ mean_age, data = mar, 
         ylab = "", xlab = "Mean age")
...
gf_barh(~fct_lump(center) %>% fct_infreq() %>% fct_rev(), data = d, ylab = "") 
```


## Factors: collapsing, custom threshold and label

```{r fig.height=3}
gf_barh(~ fct_lump(center, n = 4, other_level = "Small centers") %>% fct_rev(), 
        data = d, ylab = "") 
```
...
- The following code reads data on the dosages of a drug given to patients in a study, and redefines variable `dose` so as to contain numbers only. With this data, produce the barcharts shown below.
