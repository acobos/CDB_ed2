...
To compare survival curvces with this test, use function `surdiff()` on a formula, with the factor defining groups after the tilde.

```{r}
lung$sex <- factor(lung$sex, levels=1:2, labels = c("male", "female"))     # define factor for sex 
...
1. Use this object to fir a KM curve for each treatment group, and plot them using option `risk.table = TRUE` in the `ggsurvplot()` function call.
